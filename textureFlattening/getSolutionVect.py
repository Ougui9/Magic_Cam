...
def getSolutionVect(indexes, source, target, offsetX, offsetY, edgeMask, fuzzyCoeff):
    '''
    ...
    for i in range(h):
        for j in range(w):
            ...
            if curInd>0: # make sure it is the pixel to be replaced
                curVal = im_s[i, j]*0.8 if edgeMask[i, j]==1 else fuzzyCoeff*im_s[i, j] # current pixel val in source image
