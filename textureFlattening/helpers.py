...
'''
  Function clarification:
    ...
    - Input row: number of row of the filter you want to generate
    - Input col: number of col of the filter you want to generate
    - Input sigma: value of sigma
    - Output G/G.sum(): 2d Gaussian filter with the shape (row,col)

'''
def generateGaus2d(row, col, sigma):
    x, y = np.mgrid[-row // 2 + 1:row // 2 + 1, -col // 2 + 1:col // 2 + 1] #generate mesh grid
    G = np.exp(-((x ** 2 + y ** 2) / (2. * sigma ** 2)))
    return G / G.sum()

'''
  Function clarification:
    Return absolute angle between angle a and b
    - Input a: angle a in radians
    - Input b: angle b in radians
    - Output: absolute angle between angle a and b

'''

def smallAng(a, b):
    return abs(a-b) if abs(a-b) <= math.pi/2. else (math.pi-abs(a-b))
...
# initialize the camera
	...
	if s:  # frame captured without any errors
		...
		cv2.imwrite("userIm.jpg", img)  # save image
